...
DijetAngularAnalysisHists1::DijetAngularAnalysisHists1(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("mj1j2", "M_{jj} [GeV]", 100,0,10000);
...
void DijetAngularAnalysisHists1::fill(const Event & event){
  ...
  if(Njets>=1){
    ...
    hist("rapidity_1")->Fill(0.5*TMath::Log((event.genjets->at(0).energy()+event.genjets->at(0).v4().Pz())/(event.genjets->at(0).energy()-event.genjets->at(0).v4().Pz())), weight);
    hist("pT1")->Fill(event.genjets->at(0).pt(), weight);
    //if (event.genjets->at(0).pt() < 500) {
		//cout << " *** low jet: " << event.genjets->at(0).pt() << endl;
	//}
	...
	if( is_mc ){
		int a=-1;
		if(event.genjets->size() > 0  && event.genjets->size() >0){	
			auto kleinsterAbstand = 100000; 
			auto jet1 = event.genjets->at(0);
			...
			for (uint j=0; j<K-2; j++){
				...
				float rap1=0.5*TMath::Log((event.genjets->at(0).energy()+event.genjets->at(0).v4().Pz())/(event.genjets->at(0).energy()-event.genjets->at(0).v4().Pz()));
	...
	if(event.genjets->size() > 1 && event.genjets->size() >1){												
		//std::cout << "B" << std::endl;
		
		auto jet1 = event.genjets->at(0);
		auto jet2 = event.genjets->at(1);
		auto gjet1 = event.genjets->at(0);
		...
		if(a >= 0 && b >= 0){		
			...
			for (uint j=0; j<K-2; j++){
				...
				float ymin=yBins[j];
				float ymax=yBins[j+1];
				float rap2=0.5*TMath::Log((event.genjets->at(1).energy()+event.genjets->at(1).v4().Pz())/(event.genjets->at(1).energy()-event.genjets->at(1).v4().Pz()));
			...
			//~ 
	//~ auto pt= event.genjets->at().pt();
	//~ auto y= 0.5*TMath::Log((event.genjets->at().energy()+event.genjets->at().v4().Pz())/(event.genjets->at().energy()-event.genjets->at().v4().Pz()))
	//~ for(0.5<y<1){
		//~ if(400<pt<500){
		//~ hist(response1-> Fill(event.genjets->at()-event.genjets->at().pt())/event.genjets->at().pt(), weight);
		//~ 
	//~ }
    ...
    if(Njets>=2){
		hist("rapidity_2")->Fill(0.5*TMath::Log((event.genjets->at(1).energy()+event.genjets->at(1).v4().Pz())/(event.genjets->at(1).energy()-event.genjets->at(1).v4().Pz())), weight);
		auto rapidity1 = 0.5*TMath::Log( (event.genjets->at(0).energy()+event.genjets->at(0).v4().Pz()) / (event.genjets->at(0).energy()-event.genjets->at(0).v4().Pz()) );
		auto rapidity2 = 0.5*TMath::Log((event.genjets->at(1).energy()+event.genjets->at(1).v4().Pz())/(event.genjets->at(1).energy()-event.genjets->at(1).v4().Pz()));
		auto chi = TMath::Exp(abs((rapidity1)-(rapidity2)));
		auto jet1 = event.genjets->at(0);
		auto jet2 = event.genjets->at(1);
		
		hist("rapidity_12")->Fill(0.5*TMath::Log((event.genjets->at(0).energy()+event.genjets->at(0).v4().Pz())/(event.genjets->at(0).energy()-event.genjets->at(0).v4().Pz())), weight);
		hist("rapidity_12")->Fill(0.5*TMath::Log((event.genjets->at(1).energy()+event.genjets->at(1).v4().Pz())/(event.genjets->at(1).energy()-event.genjets->at(1).v4().Pz())), weight);
		hist("pT2")->Fill(event.genjets->at(1).pt(), weight);
		hist("mj1j2")->Fill((event.genjets->at(0).v4()+event.genjets->at(1).v4()).M(), weight); 
...
DijetAngularAnalysisHists1::~DijetAngularAnalysisHists1(){}
